...
then
	...
	miner_hashes_raw="$(tail -10 /var/run/ethos/miner_hashes.file | sort -V | tail -1)";
         echo "$current_time 详细算力:$miner_hashes_raw  总算力: $hash 低于设定值: $sethash，执行重启,成功,当天" >> /home/ethos/cron.log
    ...
    miner_hashes_raw="$(tail -10 /var/run/ethos/miner_hashes.file | sort -V | tail -1)";
    echo "$current_time 详细算力:$miner_hashes_raw  总算力: $hash 低于设定值: $sethash，执行重启成功,多天" >> /home/ethos/cron.log

    sleep 6
    cd /opt/ethos/bin && ./r 1>/dev/null 2>/dev/null
    else
	miner_hashes_raw="$(tail -10 /var/run/ethos/miner_hashes.file | sort -V | tail -1)";
         echo "$current_time 详细算力:$miner_hashes_raw  总算力: $hash  低于设定值: $sethash 已经达到当天重启上限:( $zxcs )次!等待管理员处理？" >> /home/ethos/cron.log
    fi
fi
...
then
#echo "文件执行清理!"
qlwj=$(sudo tail -10 /home/ethos/cron.log) 
